...
The research questions we intend to answer are: **for each decade, what primary game category had the highest user rating** and **between `average_playtime`, number of `users_rated`, and `max_players`, which has the greatest correlation with average rating**. To answer the first question, we will assume that the first category listed in `category` is the primary game category. We will create a new variable called `primary_category` that stores the split category column. We use `lubridate` to change `year_published` into a datetime object, with January 1st as the default date. Then we'll use `floor_date()` to find what decade each game was released in. We'll create a bar graph of the primary game category and average user rating for each decade to visualize the data. To answer our second question, we'll create three scatterplots and compare the relationships and correlation strength.
...
5.  Use `lubridate` to change `year_published` variable to date and create a `decade_published` variable

6.  Split `category` data by commas to get the primary category by using `str_split()`

7.  Change `primary_category` variable from character to a factor
...
We first create a bar graph of the primary game categories and user ratings for each decade from the 1950s to the 2010s.
...
# Apply the function to create a 'decade' factor column directly in ratings_by_category
ratings_by_category$decade <- get_decade(ratings_by_category$decade_published)
...
for (decade in unique_decades) {
  ...
  decade_ratings <- within(decade_ratings, {
    primary_category <- factor(primary_category, levels = primary_category[order(avg_category_rating, decreasing = TRUE)])
  })
  ...
  p <- ggplot(decade_ratings, aes(x = primary_category, y = avg_category_rating)) +
    ...
    theme_minimal() +
    coord_flip() +
    geom_text(aes(label = sprintf("%.2f", avg_category_rating)), vjust = -0.5, size = 3, color = "black")
  
  # Print the plot
  print(p)
}
```
From the graphs above we can see that _____________

## Research Question 2

# Create new variable `average_playtime`. 
Boardgame1 <- Boardgame1 |> 
mutate(average_playtime = ((max_playtime + min_playtime)/2))

# The First Scatterplot, `averate_playtime` vs `average_rating`
Boardgame1 |>
filter(average_playtime<7500) |>  
ggplot() +  
geom_point(aes(x=average_rating, y = average_playtime)) + 
labs (title = "Average Playtime of Board Games VS Average Rating", 
      x = "Average Rating", 
      y = "Average Playtime")
Because of outliers that made the graph extremely hard to read, some data point had to be filtered out. 

# The Second Scatterplot, `users_rated` vs `average_rating`
Boardgame1 |>
ggplot() + 
geom_point(aes(x=average_rating, y = users_rated)) + 
  labs (title = "Number of Users Rated of Board Games VS Average Rating", 
      x = "Average Rating", 
      y = "Number of Users Rated")

# The Third Scatterplot, `max_players` vs `average_rating`
Boardgame1 |> 
filter(max_players < 150) |> 
ggplot() + 
geom_point(aes(x=average_rating, y = max_players)) + 
 labs (title = "Maximum Players of Board Games VS Average Rating", 
      x = "Average Rating", 
      y = "Maximum Players")
Again, several outliers had to be filtered out to make the graph readable. 
