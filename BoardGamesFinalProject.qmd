...
6.  Split `category` data by commas to get primary category by using `str_split()`
...
## Research Question 1: Bar Graphs of Primary Game Category and Average User Rating For Each Decade

We'll first create a `decade` variable using our custom `get_decade` function and our `decade_published` variable. We take our `board_games` data and group by `decade` and `primary_category`. Within each of these groups we calculate the average category rating using `mean()`. We then order the resulting dataframe by `decade` in ascending order and then by `avg_category_rating` in descending order within each `decade` using `arrange()`. Next, we create a custom palette to apply to our bar graphs so we can differentiate between primary categories.
