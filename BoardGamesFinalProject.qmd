...
pacman::p_load(tidyverse, stringr, ggplot2, viridis)
...
for (decade in unique_decades) {
 ...
 # Generate a custom color palette with 21 distinct colors
  custom_palette <- viridis(200, alpha = 1, begin = 0, end = 1)

  # Create the plot using the selected color palette
  p <- ggplot(decade_ratings, aes(x = primary_category, y = avg_category_rating)) +
    geom_bar(stat = "identity", aes(fill = primary_category), color = "black", position = position_dodge(width = 1.0)) +
    ...
    scale_fill_manual(values = custom_palette)  # Set manual fill colors
